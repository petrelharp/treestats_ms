...
def convert_svg(svg_file):
    ...
    # Inkscape does a better job of outputing vector graphics.
    subprocess.check_call(f"inkscape {svg_file} --export-pdf={pdf_file}", shell=True)
...
if __name__ == "__main__":
    ...
    for j in range(4):
        tables.nodes.add_row(time=j + 1)

    tables.edges.add_row(0, 2, 5, 2)
    tables.edges.add_row(0, 2, 5, 1)
    tables.edges.add_row(0, 2, 6, 3)
    tables.edges.add_row(0, 2, 6, 4)
    tables.edges.add_row(0, 2, 7, 5)
    tables.edges.add_row(0, 1, 7, 6)
    tables.edges.add_row(0, 2, 8, 7)
    tables.edges.add_row(0, 2, 8, 0)
