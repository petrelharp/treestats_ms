...
In response to reviewer comments (and feedback from other sources)
we have substantially rewritten the introductory portions of the manuscript
and reworked the figures,
hopefully making it more accessible to a wider audience.
We think the manuscript is improved, and hope you agree.

In addition to the usual revised manuscript and responses to reviewers (in which page and line numbers
refer to the revised manuscript),
we have also uploaded a ``diff'' highlighting changes since the previous submission.
...
We have thoroughly revised both figures, split Figure 1 into two figures,
...
\reply{
    ...
    splitting out part (a) into \autoref{fig:adding_weights},
    simplifying it and providing more description in words.
    We have also added another figure before it
    to reinforce the same basic intuition.
    We've also rephrased the undefined phrase ``remaining weight''.
}
...
\reply{
    Our goal here was to begin with something more familiar to readers --
    the allele frequency spectrum.
    We seem to have missed the mark, based on these and other comments we received.
    (Apologies.)
    In response, we've rearranged the text, moving the description of Figure \ref{fig:divergence_diagram}
    later,
    introducing the concepts with an additional figure (Figure \ref{fig:allele_freqs}),
    and adding substantial additional text.
    We hope it is is better motivated now?
}
...
\reply{
    ...
    split out \autoref{fig:adding_weights} into a separate figure,
    rewritten the explanation,
    and begun with a different, simpler figure.
...
\reply{
    ...
    and now this example also ties in to Figure \ref{fig:allele_freqs},
...
\reply{
    Good point; we've added division by sequence length ($L$) to the figure and the caption.
}
...
\reply{
    This is true, but aren't these actually the same concept?
    For instance, the proportion of a group of samples that inherit from West Africa,
    is equal to the sum of the statistic of Example \ref{ex:ancestry_props}
    over all ``West African'' ancestors.
    We'd prefer to keep the name.
}
...
\reply{
    Unless we've gotten pdfs mixed up,
    the references actually \emph{are} ordered alphabetically by last name of first author;
    this might not be obvious because the authors are listed with first name first?
}
...
\reply{
    ...
    so we thought it best to not get into this.
}
...
\begin{point}{\revref}
    ``the genome-wide mean Branch $f_4$ is around -700''. What does that number mean? Shouldn't that be normalised by the number of sites considered? Usually, $f_4$ statistics are reported with a $Z$-score that gives statistical support for how far away from 0 the statistic is. Is there any way to do that here?
\end{point}
