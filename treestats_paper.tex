...
\plr{Question: Figures 1, 2 and 7 show the same sort of thing
    (fig 1 is a site stat, aiming to give the general overview;
    figs 2 and 7 are branch stats). Should they be more similar?
    Do we need all of these?}

\plr{right now things like $\Et\mbox{.right}$ read like a funnily terminated sentence.
     I've changed these with a macro, feel free to adjust.}
...
\plr{I assume you plan to say in words what $\beta$, $x$, $F$, etc are}
}
...
\algstep{B2.}{Terminate.}{If $j \geq |\Et|$ or $t_l \geq L$
    ...
    \plr{perhaps say "return $\sigma/L$" to make it more clear what the output is?}
}
...
\algstep{B9.}{Tree loop tail.}{Set $t_r \leftarrow L$.
    ...
    and go to \algref{B2}.
    \plr{I think you mean to add to $\sigma$, not replace it?}
}
...
\begin{figure}
    ...
    \caption{Comparison of time required to compute site statistics
        between matrix and tree based methods. [Details]
