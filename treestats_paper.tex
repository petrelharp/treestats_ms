...
% Many population genetic summary statistics can be defined using the allele frequency spectrum of a collection of homologous sequences.
...
% JK: deleted the section here as it seemed superfluous.
...
but recent methods are available to estimate them at scale.
...
correct, which no inference method can hope to achieve.
...
Firstly, we used the tree sequence estimated by \tsinfer{}
\citep{kelleher2019inferring}. By default, \tsinfer{} uses the frequency of the
derived allele at each site as a proxy for its age (and, therefore, the age
corresponding ancestor and node), aiming only to estimate accurate
tree topologies. We therefore expect the correspondence between branch
length and site statistics in this case to be poor. Secondly,
we used allele age estimates for each site supplied by GEVA~\citep{albers2019dating},
as input to \tsinfer{}. In this case, ancestor ages were determined by
the GEVA allele age estimates rather than the default frequency
based heuristic used by \tsinfer{}; inference was run as usual
otherwise. Thirdly, we used trees and node times estimated
...
As expected, branch diversity from \tsinfer{} shows the worst agreement with
site diversity.
The two methods that infer node times differ substantially,
with Relate showing a significantly tighter agreement than
GEVA+\tsinfer{}.
Does this mean that Relate is doing a better job at inferring the tree sequence
than GEVA+\tsinfer?
Answering this question requires a deeper understanding of the processes that shape
genetic diversity along the genome.

\jk{I'm not particularly happy with this to be honest: it's all very
hand-wavey. Can we be more precise about the correspondence between
site and branch statistics? Relate looks a bit better, but I don't
think we can say "significantly" (sounds like p-values or something).
Is there some sort of time-series analysis we could do to show how
similar sets of squiggly lines are to each other?}

\krt{I have related concerns. What does it mean to look at a branch stat and a site state
    from an inferred tree sequence?  The input genotype data are informative about both 
    the inferred edges and their branch lengths.   So it seems to me that comparing the two types of stats is comparing
    non-independent quantities, and I am unclear how to interpret that? (I admit to not studying the GEVA paper closely
yet, nor the Speidel).  Presumably, the site stat is going to give you the right value (e.g., match whatever the
original paper reported for, say, pi), but the branch version of the stat could differ from this for perhaps a lot 
of reasons that all contribute to "edge estimation error"?  Not calling variants below a certain frequency,
preferentially calling variants you've seen in a previous study, or imputing in variants from other studies?
Overall, I have to squint at Fig S2 to see differences, and most of the issue seems to be near what I assume to be a
centromere?}
...
For larger sample sizes, there is an uptick in the time required per variant
by \texttt{tskit} shown in \autoref{fig:stats_performance}.
This occurs because the $O(n)$ complexity of building the first tree
begins to dominate run time for large $n$,
...
or about biases of the inference methods,
...
\begin{figure}
    ...
    \caption{
        ...
        \krt{This should be tsinfer+GEVA, no?}
