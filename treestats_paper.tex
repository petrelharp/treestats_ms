...
We exploit this correspondence here to define a general framework that
efficiently compute single-site population genetic statistics
...
genotype matrix simply cannot fit into memory, and complex distributed computations
are required.
...
We will often be interested in statistics defined in terms of different
subsets of our samples (corresponding to, e.g., different populations); it
is useful to have some additional notation to define these statistics.
...
at a site, and we may define a \emph{polarized} node statistic without the second term from our previous definition:
...
However, the correspondence between the polarized statistics is less tight
than for the unpolarized ones.

\jk{What does this last sentence mean?}
...
scale~\citep{harris2019database,kelleher2019inferring,speidel2019method}.
...
or just a byproduct of the inference method (which, naturally, estimates branch lengths to be proportional to numbers of mutations)?
Answering this question requires a deeper understanding of the processes that shape
genetic diversity along the genome.


\begin{figure}
    \includegraphics{{figures/1kg/relate_chr20.site.1000000.region.diversity}.pdf}
    \includegraphics{{figures/1kg/relate_chr20.branch.1000000.region.diversity}.pdf}
    \caption{
        ...
        Note that the Site statistic divided by the Branch statistic is close to $2 \times 10^{-8}$,
        the mutation rate used by Relate \plr{XXX total speculation here}.
        \label{fig:chr20}
    }
\end{figure}
...
This generality was partially motivated by laziness ---
implementing new statistics is now simply a matter of devising the appropriate summary function ---
but it also allows us to easily explore new classes of statistics.
...
which provides a suite tools for efficiently working with tree sequences in Python and C.
...
\citep[see, e.g.,][]{martin2017human,browning2018one,galloway2019stickleback}.
% TODO rephrase this: large scale simulations are useful for lots of things.
% No need for the slightly negative note about large datasets only being
% relevant for humans, I think.
...
may use the signal from nearby patterns of variation
...
and so could also be used to summarize contributions to various statistics across \emph{time}.
...
and Leo Speidel for providing the inferred 1000 Genomes tree sequence.
