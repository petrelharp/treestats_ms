...
In other words, if the frequency of the derived allele at a site $i$ is $p_i$,
and the derived allele frequency spectrum
...
$\sum_p f(p) h(p) = (1/L) \sum_{i=1}^L h(p_i)$.
(For the moment we ignore sites with more than two alleles.)
...
are the ``weights''.
For instance, we could calculate for a given tree how many of 
a certain set $S$ of samples are below each of its edges
...
\begin{definition}[Subtree weights]
    ...
    \begin{align*}
        \nw(u) = \sum_{v \,:\, v \le_T u} \iw(v) ,
    \end{align*}
    where $v \le_T u$ if $u$ is on the path from $v$ back to the root of $T$.
\end{definition}

We will use weights of the form described above frequently enough
that we give them a special name --
for a set $S$ of samples,
the \emph{indicator weights of $S$} are the weights $\bone_S$ with
...
If all sites are biallelic, and the sum of all weights is $\bar \iw = \sum_u \iw(u)$,
then any statistic defined for an $f$ with $f(x) = f(\bar \iw - x)$
will in fact not change if the identity of ancestral/derived alleles is flipped.
...
This definition is simple, but we make no claims that it is the best or most natural one.
...
\textbf{branch statistic} associated with the branch of length $t$
