...
Let $p = g^T g$ be the allele frequency, 
let $u = g^T Z$ be the vector giving sums of the covariates of all samples carrying the allele,
and
\begin{align*}
    \alpha
    &=
        (g^T g - g^T Z (Z^T Z)^{-1} Z^T g)^{-1} \\
    &=
        \frac{1}{
            p - \sum_j u_j^2
        } .
\end{align*}
By the inversion formula for a block two-by-two matrix,
...
% stopifnot(all(abs(- alpha * u - solve(crossprod(B))[1,-1]) < 2e-15))
% stopifnot(abs(solve(crossprod(B))[1,1] - alpha) < 2e-15)
