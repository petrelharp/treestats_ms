...
Summaries of genetic variation thus also describe the underlying genealogies.
...
and to suggest several new statistics.
...
It was once a major undertaking to collect data sufficent to estimate a single summary
...
Bypassing this bottleneck will require new data structures appropriate for genomic data.
...
where similar efficiency gains were made. 
Recent work~\citep{kelleher2018inferring} has shown that tree sequence algorithms can
also be used to massively increase the scalability of methods for inferring
genome-wide genealogies, and making it possible to infer trees for millions of whole genome samples.
...
\citet{kelleher2016efficient} used this redundancy
...
This paper generalizes the strategy used by \citet{kelleher2016efficient} for statistics computation
...
are also described by the Ancestral Recombination Graph \citep[or ARG,][]{arg},
the basis for several inference methods \citep{argmethod}.
These genealogical trees can be fairly precisely inferred in nonrecombining regions
\citep{felsenstein_book},
...
This paper presents one way of generalizing of this idea.
...
\begin{align}
    \branch(f, \iw)_T
    &=
    \sum_{u \in T} t_{u} \left( f(\nw_{T}(u)) + f(\tiw - \nw_{T}(u)) \right)  ,
\end{align}
where $t_{u}$ is the length of the branch ancestral to node $u$ in tree $T$
...
The most direct effect of the methods presented here might be on efficiency of computation
with large genomic datasets --
the python library \tskit{} provides fast, rigorously tested implementations
of many standard population genetic statistics, with more on the way.
The logarithmic scaling of storage and computation time with sample size
could bring much more computation-intensive methods within reach on modern large genomic datasets
of humans, 
and helps make detailed investigation of whole populations in evolutionary simulations feasible.

% but the duality thing is maybe more long-term exciting
However, the focus of this paper was not on computation of standard population genetics statistics,
but on the relationship of these statistics to the underlying trees.
We have used essentially the same general framework
to define two additional classes of statistics -- Branch and Node statistics --
that well-defined relationships to the Site statistics computable from genome sequence.
% describing trees is cool
At least, this gives us descriptive tools to directly describe the genealogical trees
of tree sequences obtained by simulation.
% and we can do it with real data now
However, new methods to infer tree sequences allow us to apply these methods to real data.
% branch stats are less noisy!
Given the true tree sequence,
Branch statistics are less-noisy versions of the standard Site statistics,
so methods that use Branch statistics may show increased power and precision
over the corresponding genotype-only methods.
We have shown that this is true in simulated data,
but it is not yet known how much additional noise the process of tree sequence inference will add.
% and we get the time dimension
Furthermore, tree sequences add a \emph{time} dimension to genotype data,
which these descriptive methods open up many possibilities for utilizing and visualizing of.
