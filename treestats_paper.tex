...
individuals~\citep[e.g.,][]{browning2010highresolution,speed2014relatedness,aguillon2017deconstructing},
and within
...
projects on the scale of the UK Biobank~\citep{bycroft2018genome}
...
these genotype matrices will be very costly.

The genotype matrix, fortunately, is not the only way to encode genetic data.
...
these cost of exporting data to a genotype matrix will in practice greatly
speed up applications such as deep learning \jk{cite} and ABC~\jk{cite}
that compute many statistics from simulated data. The ability to
efficiently compute statistics from real data is also welcome, of course.
