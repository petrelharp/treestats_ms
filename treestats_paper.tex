...
In this section we provide describe and analyse the algorithm used
to compute branch length statistics. This is a generalation
of the algorithm used to maintain the number of samples from a
given set that derive from each node in a tree sequence.
\cite[Algorithm L]{kelleher2016efficient}.

\jk{Plan for section}
\begin{itemize}
\item First paragraph is a general outline of the strategy.
\item Explanation of notation and the index vectors (we should probably define
these again, in the interest of completeness. I'm not sure the definition
in the msprime paper really worked)
\item Algorithm listing.
\item Next para is an explanation of the algorithm in words, referring
to the figure for what we mean by adding and removing edges.
\item Next para is an analysis of the algorithm. We note here that
we propagate stuff all the way up the tree close to optimally, because of the
ordering of edge insertions and removals. For example, we always disconnect
a subtree from the main tree first, meaning that the only time we go all the
way to the root is the first disconnection.
Discuss the figure perf figure.
\end{itemize}
...
\algstep{B2.}{Terminate.}{If $j \geq |\Et|$ or $t_l \geq L$
    set $\sigma \leftarrow \sigma / L$ and terminate.
}

\algstep{B3.}{Edge removal loop.}{If $k \geq |\Et|$ or $t_l \neq
    \Et[\indexout_k]\mbox{.right}$ go to \algref{B6}}.

\algstep{B4.}{Remove edge.}{Set
    $u \leftarrow \Et[\indexout_k]\mbox{.child}$,
    $v \leftarrow \Et[\indexout_k]\mbox{.parent}$ and $k \leftarrow k + 1$.
...
\algstep{B6.}{Edge insertion loop.}{If $j \geq |\Et|$ or $t_l \neq
    \Et[\indexin_j]\mbox{.left}$ go to \algref{B9}}.

\algstep{B7.}{Insert edge.}{Set
    $u \leftarrow \Et[\indexin_j]\mbox{.child}$,
    $v \leftarrow \Et[\indexin_j]\mbox{.parent}$ and $j \leftarrow j + 1$.
    Then set $\pi_u \leftarrow v$,
    $\beta_u \leftarrow \Nt[v]\mbox{.time} - \Nt[u]\mbox{.time}$ and
...
\algstep{B9.}{Tree loop tail.}{Set $t_r \leftarrow L$.
    If $j < |\Et|$ set $t_r \leftarrow \min(t_r, \Et[\indexin_j]\mbox{.left})$.
    Then, if $k < |\Et|$ set $t_r \leftarrow \min(t_r, \Et[\indexout_k]\mbox{.right})$
    Finally, set $\sigma \leftarrow (t_r - t_l)s$,  $t_l \leftarrow t_r$
    and go to \algref{B2}.
}
...
\begin{tabular}{cccc}
\includegraphics[width=3cm]{figures/tree_0_in_6.pdf} &
\includegraphics[width=3cm]{figures/tree_0_out_0.pdf} &
\includegraphics[width=3cm]{figures/tree_0_out_1.pdf} &
\includegraphics[width=3cm]{figures/tree_1_in_0.pdf}
\\
...
\caption{Example of the algorithm state during the transition between two
trees. \jk{If we keep this then we talk about what we see in the different
examples. We want to give some insight into how the algorithm works by showing
