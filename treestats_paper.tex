...
$n$ and population scaled recombination rate $\rho$. Under these conditions,
we know that there are $O(n + \rho \ln n)$ edges in the tree
sequence~\cite{kelleher2016efficient}. Clearly, each edge is examined
...
\[
O(n \log_2 n + \rho \log_2 n \ln n)
\]
\jk{Slightly iffy on what this should be in $O$ terms. I'm pretty
sure that the total number of edges of log base $e$ and the
expected height is log base 2. Not sure what the asymptotics
say here.}
...
\begin{figure}
    ...
    \caption{Comparison of time required to compute site statistics
        ...
        scaled recombination rate $\rho = 4N_er$ both equal to $10^4$. For each replicate, Tajima's $D$
