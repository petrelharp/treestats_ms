...
can cause the precise set of samples that inherit from that edge to differ across
...
\begin{example}[Ancestry proportions] \label{ex:ancestry_props}
    ...
    and then $\iw_u / n$ is the proportion of the samples in $S$ that inherit from $u$.
...
\begin{example}[Phenotypic correlations] \label{ex:site_correlations}
    ...
    Appendix \ref{apx:regression} explains how to extend this to regression with covariates.
\end{example}
...
\begin{example}[Phenotypic correlation with pedigree] \label{ex:branch_correlation}
    ...
    Similar strategies for using local tree topology to improve associate mapping
    were explored by \citet{zollner2005coalescent} and \citet{minichiello2006mapping}.
\end{example}
...
\krt{Is there a circularity here?  What does a branch statistic from real data mean
when the branch lengths must be inferred somehow from the mutations? If this is not circular,the 
reasons why need to be made clearer.}
...
Does this mean that Relate is doing a better job at inferring the tree sequence than \texttt{tsinfer} pluse GEVA?
The results presented here are highly suggestive, but not entirely clear.
...
\begin{figure}
    ...
    \caption{
        ...
        \krt{the only appearances of "dual branch" are in figure legends.  need to define somewhere}
...
\krt{Not sure exactly how to rewrite.}
...
where $p_A$ is the proportion of samples in $A$ that inherit from that node, etcetera.
...
of Algorithms~\algref{B} is of order $(n = \rho \ln n)\log_2 n$, which is
\[
    O\left(n \log n + \rho \log^2 n \right)
\]
